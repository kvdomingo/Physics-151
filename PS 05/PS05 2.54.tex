...
\begin{equation}\label{eq:thirdlaw}
	S = k_B \log \Omega
\end{equation}

If we assume that $S$ is of the order $Nk$, where $N$ is the number of particles in the black hole, and that all the particles are photons, \eqref{eq:thirdlaw} has the form

\begin{equation}
	S = k_B \frac{M}{m_\gamma}
\end{equation}

where $k_B$ is Boltzmann's constant. The black hole's entropy is

\begin{equation}
	S = k_B \frac{Mc\lambda}{h}
\end{equation}

Plugging \eqref{eq:wavelen} into this,
